          ...
          {globalRate !== null && (
            <div className="mt-3 rounded-xl border border-border bg-card p-4">
              <div className="mb-1.5 flex items-center justify-between text-xs">
                ...
                <span className={cn('text-sm font-bold tabular-nums', globalRate >= 90 ? 'text-emerald-600' : globalRate >= 75 ? 'text-amber-600' : 'text-red-600')}>
                  {globalRate}%
                </span>
              </div>
              <div className="h-2 overflow-hidden rounded-full bg-muted">
                <div
                  className={cn('h-full rounded-full transition-all duration-300', globalRate >= 90 ? 'bg-emerald-500' : globalRate >= 75 ? 'bg-amber-500' : 'bg-red-500')}
      ...
      {!loading && report && report.rows.length === 0 && (
        <div className="rounded-2xl border border-dashed border-border p-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Chaque appel fait depuis l&apos;onglet « Faire l&apos;appel » apparaîtra automatiquement ici.
          </p>
        </div>
      )}
      ...
      {!loading && visibleRows.length > 0 && (
        <div className="space-y-2">
          {visibleRows.map((row, index) => {
            ...
            return (
              <FadeIn key={row.student.id} delay={Math.min(index, 8) * 30} from="bottom">
                <article className={cn('rounded-xl border bg-card transition', isOpen ? 'border-primary/40 shadow-sm' : 'border-border hover:border-primary/30')}>
                  <button
                    type="button"
                    onClick={() => setExpandedStudent(isOpen ? null : row.student.id)}
                    className="flex w-full flex-wrap items-center gap-x-4 gap-y-2 p-3 text-left sm:p-4"
                    aria-expanded={isOpen}
                  >
                    <span className="text-lg">{row.group?.emoji ?? '📋'}</span>
                    <div className="w-44 min-w-0 flex-1 sm:flex-none">
                      <Link
                      ...
                      <p className="truncate text-xs text-muted-foreground">{row.group?.name ?? '—'}{row.group?.site ? ` · ${row.group.site}` : ''}</p>
                    </div>
                    ...
                    <div className="ml-auto flex items-center gap-3">
                      <div className="w-24">
                        <div className="h-1.5 overflow-hidden rounded-full bg-muted">
                          <div
                            className={cn('h-full rounded-full', row.rate >= 90 ? 'bg-emerald-500' : row.rate >= 75 ? 'bg-amber-500' : 'bg-red-500')}
                            style={{ width: `${Math.max(row.rate, 3)}%` }}
                          />
                        </div>
                      </div>
                      <span className={cn('w-11 text-right text-sm font-bold tabular-nums', row.rate >= 90 ? 'text-emerald-600' : row.rate >= 75 ? 'text-amber-600' : 'text-red-600')}>
                        {row.rate}%
                      </span>
