...
// Retourne : { students: [...], attendance: [...], stats }
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Récupère la session + son groupe
    ...
    // Élèves inscrits dans ce groupe (enrollments actifs)
    ...
    // Présences déjà enregistrées pour cette session
...
// Upsert en masse — remplace les enregistrements existants
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Retourne les absents pour proposer notifications
    const absents = (data ?? []).filter((r) => r.status === 'absent')

    return NextResponse.json({
      saved:   data?.length ?? 0,
      absents,
      message: `${data?.length ?? 0} présences enregistrées`,
    })
  } catch (e) {
    return NextResponse.json({ error: String(e) }, { status: 500 })
  }
