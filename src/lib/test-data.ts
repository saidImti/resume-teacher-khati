...
export async function getTestDataStatus(admin: AnySupabase): Promise<TestDataStatus> {
  const [{ count: families }, { count: students }, { count: enrollments }] = await Promise.all([
    admin.from('families').select('*', { count: 'exact', head: true }).ilike('notes', TEST_TAG_MATCH_PATTERN),
    admin.from('students').select('*', { count: 'exact', head: true }).ilike('notes', TEST_TAG_MATCH_PATTERN),
    admin.from('enrollments').select('*', { count: 'exact', head: true }).ilike('notes', TEST_TAG_MATCH_PATTERN),
  ])
...
export async function generateTestStudents(admin: AnySupabase, opts: GenerateOptions): Promise<GenerateResult> {
  ...
  const [{ data: sites }, { data: levels }, { data: years }, { data: userRow }] = await Promise.all([
    ...
    admin.from('levels').select('id, name, age_min, age_max, sort_order').order('sort_order'),
    admin.from('academic_years').select('id, name, is_active'),
    admin.from('students').select('user_id').limit(1).single(),
  ])
  if (!userRow?.user_id) throw new Error("Impossible de déterminer l'utilisateur (aucun élève existant en base pour dériver user_id).")
  ...
  const { data: existingGroups } = await admin.from('groups').select('id, site_id, level_id')
  const { data: existingSchedules } = await admin.from('schedules').select('id, group_id, day_of_week')
    ...
    for (const level of levels ?? []) {
      ...
      if (foundGroup) {
        ...
        const { data: newGroup, error } = await admin
          .from('groups')
          .insert({ site_id: site.id, level_id: level.id, academic_year_id: activeYear.id, name: level.name, is_active: true })
          .select('id')
          .single()
      ...
      for (const day of [WED, SAT]) {
        ...
        const { error } = await admin.from('schedules').insert({
          user_id: userId, group_id: groupId, site_id: site.id, day_of_week: day,
      ...
      for (let i = 0; i < studentsPerGroup; i++) {
        ...
        const { data: fam, error: famErr } = await admin.from('families').insert({
          user_id: userId, parent1_first: `Parent-${prenom}`, parent1_last: nom,
        ...
        const { error: enrErr } = await admin.from('enrollments').insert({
          user_id: userId, student_id: stu.id, group_id: groupId, academic_year_id: activeYear.id,
...
export async function purgeTestData(admin: AnySupabase): Promise<PurgeResult> {
  const { data: enr } = await admin.from('enrollments').delete().ilike('notes', TEST_TAG_MATCH_PATTERN).select('id')
  const { data: stu } = await admin.from('students').delete().ilike('notes', TEST_TAG_MATCH_PATTERN).select('id')
  const { data: fam } = await admin.from('families').delete().ilike('notes', TEST_TAG_MATCH_PATTERN).select('id')
