...
> Dernière mise à jour : **2026-07-07** (v5.0 — 🚧 **PIVOT SaaS MULTI-TENANT en cours** sur la branche `feat/multi-tenant-saas` : table `organizations`, isolation complète par école, inscription libre. Voir §17 sessions 21-22, [`CHANTIER_MULTI_TENANT.md`](./CHANTIER_MULTI_TENANT.md) sur la branche, et §28.)

> ✅ **Checkpoint de continuité (2026-07-07)** : `main` est propre et déployé (Vercel `success`, dernier commit `main` @ `35cec82` — fixes impression : signature par page de groupe, taille/position, marges garanties). **Le chantier multi-tenant vit sur la branche `feat/multi-tenant-saas`** (commit `3f972eb`, poussée sur GitHub — rien ne se perd au redémarrage). ⚠️ **NE PAS merger cette branche avant d'avoir appliqué `018_organizations.sql`** (SQL Editor) : le code de la branche lit la table `organizations` qui n'existe pas encore. État exact + ordre de reprise : [`CHANTIER_MULTI_TENANT.md`](./CHANTIER_MULTI_TENANT.md) (sur la branche). Seul `Signature Teacher Khati.png` (fichier personnel, racine) reste volontairement non commité. **À noter** : ni logo ni signataire réels en base à ce jour (vérifié) — l'utilisateur n'a pas encore refait l'upload ; après le chantier multi-tenant, l'upload passera par l'organisation.
> **Prochaine action** : reprendre le chantier multi-tenant — `git checkout feat/multi-tenant-saas` puis suivre la section « ⬜ RESTE À FAIRE » de CHANTIER_MULTI_TENANT.md (étape 1 : appelants marque).
...
| `018_organizations.sql` | 🚧 Multi-tenant : organizations + organization_id partout + RLS par org | ⬜ **PAS appliquée** (branche `feat/multi-tenant-saas`) |

> ✅ Migrations 001–017 appliquées, fonctionnalités opérationnelles. ⚠️ La 018 est écrite mais **pas appliquée** — à passer dans le SQL Editor AVANT de merger la branche multi-tenant (voir `CHANTIER_MULTI_TENANT.md`).
