...
> Dernière mise à jour : **2026-07-04** (v4.2 — PDF présence + appel du jour groupé finalisés en A4 Paysage/1 page par groupe, pages d'impression sorties du layout app, Marque : logo + signataires configurables. Voir §17 session 18 et §28.)

> ✅ **Checkpoint de continuité (2026-07-04)** : tout le travail listé ci-dessous est **committé, poussé et déployé en production** (Vercel `success` sur chaque PR, dernier commit `main` @ `987c826`). Un redémarrage de poste ou une nouvelle session peut reprendre directement depuis ce document sans rien perdre — aucun travail local non sauvegardé. Seul `Signature Teacher Khati.png` (fichier personnel de l'utilisateur à la racine du projet) reste volontairement non commité — plus nécessaire de le committer, il s'uploade désormais depuis `/settings/marque`.
