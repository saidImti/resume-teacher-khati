...
> Dernière mise à jour : **2026-07-05** (v4.3 — Fix UX critique sur `/settings/marque` : ambiguïté « Ajouter » faisait croire un signataire enregistré alors que rien n'était sauvegardé. Voir §17 session 19 et §28.)

> ✅ **Checkpoint de continuité (2026-07-05)** : tout le travail listé ci-dessous est **committé, poussé et déployé en production** (Vercel `success` sur chaque PR, dernier commit `main` @ `ce70b1a`). Un redémarrage de poste ou une nouvelle session peut reprendre directement depuis ce document sans rien perdre — aucun travail local non sauvegardé. Seul `Signature Teacher Khati.png` (fichier personnel de l'utilisateur à la racine du projet) reste volontairement non commité — s'uploade désormais depuis `/settings/marque` (bouton **« Enregistrer ce signataire »**, sans ambiguïté depuis le fix du 2026-07-05).
