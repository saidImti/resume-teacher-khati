...
> Dernière mise à jour : **2026-07-14** (v6.0 — 🎉 **`feat/multi-tenant-saas` fusionnée sur `main` et déployée sur Vercel**, sur demande explicite de l'utilisateur, malgré des points de vérification non terminés — voir avertissement ci-dessous. Contient : le pivot SaaS multi-tenant complet (migration 018, isolation par organisation, rôles) ET le travail de fusion legacy fait sur la même branche (formulaire d'inscription dynamique, page `/settings/tarification`, 4 corrections de calcul tarifaire). Voir §17 sessions 21-23.)

> ⚠️ **Points non vérifiés au moment du merge (2026-07-14)** — fusionné à la demande explicite de
> l'utilisateur avant la fin du plan de vérification prévu dans `CHANTIER_MULTI_TENANT.md` :
> QR d'inscription publique (nouveau format organisationId+userId ET ancien format legacy userId
> seul), marque (logo/signataires) par organisation en conditions réelles, et surtout **aucune
> confirmation avec le VRAI compte Teacher Khati de l'utilisateur** que rien n'a régressé — tout
> n'a été testé qu'avec des comptes jetables. À vérifier en priorité après ce déploiement.
...
| `018_organizations.sql` | 🚧 Multi-tenant : organizations + organization_id partout + RLS par org | ✅ **APPLIQUÉE (2026-07-08)** + 2 correctifs post-application (trigger signup recréé, unique `academic_years.is_active` re-scopé par org — voir `ERRORS/002` et `ERRORS/003` sur la branche) |

> ✅ Migrations 001–018 appliquées. ⚠️ Le CODE multi-tenant correspondant est encore sur la branche `feat/multi-tenant-saas` (pas mergée) — la base est en avance sur le code déployé, compatibilité assurée par les triggers org-fill. Migration 019 (nettoyage) à écrire après le merge.
...
### Immédiat — EN COURS (2026-07-07)
Bilan qualité "grandes écoles" fait le 2026-07-02. Décision tranchée le 2026-07-06 : le point « multi-utilisateurs » a **pivoté en chantier SaaS multi-tenant complet** (l'utilisateur prépare un SaaS — chaque client/école aura logo, nom et données isolés).
- [~] 🚧 **Multi-tenant SaaS (organizations)** — EN COURS sur `feat/multi-tenant-saas` : table `organizations`, `organization_id` sur les 26 tables, RLS par org, rôles admin/teacher/viewer, inscription libre self-service, marque par organisation. **Référence : [`CHANTIER_MULTI_TENANT.md`](./CHANTIER_MULTI_TENANT.md)** (sur la branche). Absorbe et remplace l'ancien point « multi-utilisateurs / rôles ».
- [ ] **Listes d'attente + conversion des essais** — pas de suivi des prospects en attente de place.
- [ ] **Journal d'activité** — aucune trace de qui a modifié quoi (encore plus utile en multi-tenant).
- Secondaires notés mais pas priorisés : documents/autorisations par famille, portail parents (flag prêt, UI à faire), statistiques de rétention, WhatsApp Business réel (encore test_mode), vérifier le plan de sauvegarde Supabase (point-in-time recovery), migration 019 de nettoyage post-multi-tenant (drop `users.logo_url`, uniques `user_id` résiduels).
...
| 21 | 2026-07-06 | **Finitions impression registre** (PR #20, #21) : signature agrandie (96 px/260 px) et alignée à gauche — le décalage `translateX(-30mm)` demandé initialement mesuré à 0,2 mm du bord imprimable (risque de rognage), remplacé par un alignement sûr à 1 mm ; **marges d'impression garanties** : `@page margin` ignoré par Chrome selon le réglage du dialogue → marges posées en dur via padding du document + `padding-top` sur chaque saut de page (chaque page a sa marge haute, pas seulement la première) |
| 22 | 2026-07-06/07 | 🚧 **PIVOT SaaS MULTI-TENANT — chantier ouvert** (branche `feat/multi-tenant-saas` @ `3f972eb`) : décisions actées (multi-tenant complet, inscription libre, backfill org « Teacher Khati », rôle viewer, marque par organisation) ; audit préalable par 3 agents (inventaire RLS des 26 tables, tous les points d'INSERT) ; **4 bugs préexistants découverts** (escalade de privilège withApiAuth, /api/users listait toute l'instance, PATCH rôle sans effet RLS, inscription publique QR cassée) ; FAIT : migration 018 complète (non appliquée), plomberie org, signup self-service, /api/users org-scopé, branding.ts org-level ; RESTE : voir `CHANTIER_MULTI_TENANT.md` |
| 23 | 2026-07-08/09 | 🚧 **Multi-tenant — étapes 1-5 terminées + E2E entamée** (branche @ `7a4efc3`) : marque org-level (routes logo/signataires, Sidebar, prints), routes de données scopées, gating viewer UI, **migration 018 APPLIQUÉE en base** (+ 2 correctifs post-application) ; vérification E2E : **5 bugs débusqués et corrigés** dont 2 critiques — **fuite cross-tenant réelle** (`queries.ts` + ~20 pages Server Component sans scoping org : une org neuve voyait les données de Teacher Khati) et **invitation cassée** (le trigger ne voit pas `app_metadata` à temps, l'invité créait sa propre org — corrigé côté `POST /api/users`) ; **isolation validée en réel** (dashboard org neuve = 0 partout) et **rôles validés en réel** (viewer 403 API + UI, teacher 201 pédagogique / 403 finances) ; nouveau dossier `ERRORS/` (catalogue des bugs avec diagnostic/solution) ; RESTE : numéros d'inscription par org, QR nouveau+legacy, marque en réel, reconnexion vraie de l'utilisateur, merge+deploy+019 — voir « 🔖 REPRISE ICI » de `CHANTIER_MULTI_TENANT.md` |
| 24 | 2026-07-11/12 | **Code d'inscription par site en UI + formulaire d'inscription dynamique multi-enfants** (branche `feat/multi-tenant-saas`) : `sites.registration_prefix` exposé et éditable inline sur la carte du site (auto-suggestion, anti-doublon) ; fix critique rattachement famille existante cassé (`family.id` manquant dans `getStudents`) ; fix critique double-soumission créant des familles dupliquées (verrou par ref). Puis réécriture complète de `/eleves/new` : l'assistant 4-étapes-par-élève remplacé par une page unique dynamique reprenant le pattern legacy (parent saisi une fois, enfants ajoutés à la volée) mais branchée sur le **vrai** moteur `pricing_rules` de RTK (5 paliers, 3 modes de facturation) au lieu d'un tableau figé — tarif recalculé en direct à chaque champ. 3 modes : Nouvelle famille, Ajouter à une famille existante (rang dégressif correctement décalé selon la fratrie déjà présente), **Rentrée — élèves connus** (nouveauté vs legacy : RTK garde un élève sur plusieurs années via `enrollments`, donc réinscrire ne duplique rien — juste une nouvelle inscription groupe/année sur le dossier existant, niveau recalculé selon l'âge). Détection de doublon en direct pendant la saisie. Nouvelles routes `GET /api/pricing-rules?siteId`, `GET /api/families/search`, `GET /api/families/[id]/students`. Tout vérifié en conditions réelles (pas seulement `tsc`) avec compte de test jetable, données nettoyées après coup. **Gap découvert au passage** : aucune UI pour configurer `pricing_rules` (seule une route `POST` existe, seedée aujourd'hui via script admin pour les tests) — à construire avant que cette fonctionnalité soit utilisable en pratique par un admin. |
| 25 | 2026-07-13/14 | **Fix critique tarif dégressif + page `/settings/tarification`** (branche `feat/multi-tenant-saas`) : le formulaire d'inscription (session 24) calculait le dégressif comme un barème progressif (1er enfant 40€, 2e 35€, 3e 30€ = 105€ pour 3 enfants), alors que le legacy (`getMensualite`) et `generate-monthly/route.ts` appliquent un **tarif unique par enfant** selon la taille de la fratrie (3 enfants → 30€ chacun = 90€). Le formulaire affichait donc un montant différent de ce qui serait réellement facturé. Extrait la logique dans `src/lib/pricing.ts` (`unitRateForFamilySize`, `monthlyForFamily`) pour éviter une 3e duplication, `generate-monthly` refactorisé pour l'utiliser. Puis construit `/settings/tarification` (onglet nav ajouté) : ni RTK ni le legacy n'avaient de vraie UI pour ça (le legacy le listait dans sa propre roadmap sans jamais la construire) — 2 onglets, "Par site" (carte par site, édition inline, stats familles/enfants/mensuel/annuel/%CA comme le legacy) et "Par famille" (recherche + tarif spécial `custom_monthly_rate`, priorité absolue). Réutilise entièrement l'API existante, aucune nouvelle route. Vérifié en conditions réelles : tarif configuré et persisté par palier, stats recalculées après inscription d'une famille test. |
| 26 | 2026-07-14 | **Fix critique 4e duplication de calcul tarifaire (forfait famille traité comme dégressif) + suppression du CRUD tarifs dupliqué** : `computeMonthlyAmount()` (`queries.ts`) — consommé par `FinancesContent.tsx`, le registre `school-register.ts` et l'estimation de prix de `public-registration/route.ts` — multipliait encore le tarif `monthly_family` (forfait fixe) par le nombre d'enfants, contredisant `generate-monthly/route.ts`. Bug préexistant, pas introduit en session 25. Corrigé par délégation complète à `monthlyForFamily()` (`lib/pricing.ts`, source de vérité unique, `sessionsInMonth` ajouté à sa signature). Fix de la carte "Vue d'ensemble" de `FinancesContent.tsx` : le libellé et l'affichage étaient inversés entre les 2 modes. Vérifié en conditions réelles avec compte jetable : site en forfait 100€/famille, 2 enfants inscrits → Finances et registre familles affichent bien 100€ (pas 200€). Puis, décision utilisateur sur la duplication architecturale signalée (« si il faut retirer pour faire encore mieux et plus puissant et dynamique alors vas-y ») : **CRUD tarifs de `FinancesContent.tsx` retiré** (formulaire création tarif par site, formulaire tarif spécial famille, boutons Modifier/Retirer du registre solidaire — ~290 lignes), l'onglet "Tarifs & aides" devient une vue lecture seule (grilles par site, registre des tarifs spéciaux) avec un bandeau CTA renvoyant vers `/settings/tarification`, seule surface d'édition restante. Vérifié en navigateur (2 comptes jetables, `tsc` propre, capture d'écran de l'onglet simplifié + navigation vers la page dédiée). |
| 27 | 2026-07-14 | 🎉 **Fusion `feat/multi-tenant-saas` → `main` + déploiement Vercel**, sur demande explicite de l'utilisateur (« implémente tout et ne me demande pas d'autorisation »), avant la fin du plan de vérification du chantier (voir avertissement en tête de document). |
| 28 | 2026-07-14 | 🔴 **Fix critique post-déploiement : dashboard bloqué en chargement infini avec un vrai compte**. L'utilisateur a signalé le blocage juste après le merge (session 27). Diagnostic en 2 temps : (1) logs runtime Vercel → boucle `/dashboard (200) → /api/academic-years (401) → /auth/login (307) → /dashboard (200)` en continu ; base de données vérifiée saine (script direct, 169 élèves/5 sites, réponses < 1s) ; cause identifiée comme une course sur le rafraîchissement du refresh token Supabase (plusieurs appels indépendants à `auth.getUser()` par requête — middleware + `getOrgContext()` + fetch client). Fix : le middleware devient l'unique vérificateur par requête HTTP, transmet l'identité vérifiée via un header interne (`x-mw-verified-user-id`, jamais exposé/falsifiable) que `getOrgContext()`/`withApiAuth()` consomment au lieu de revérifier. (2) Après déploiement, le symptôme persistait (401 systématique, pas intermittent) : un flag de debug temporaire a confirmé que le header était bien transmis — la vraie cause était **2 comptes `auth.users` sans ligne `public.users`** (orphelins pré-existants, jamais comblés par le backfill de la migration 018 qui ne peut qu'`UPDATE`, pas créer une ligne manquante). Lignes `public.users` créées manuellement pour ces 2 comptes. Dashboard vérifié fonctionnel de bout en bout après ce 2e fix (vraies données : 5 sites, 20 groupes, 169 élèves). Détail complet dans `ERRORS/008`. |
| 29 | 2026-07-15/16 | **Tarification — options étendues, reprise d'un travail interrompu en session précédente** (migration 019, `lib/pricing.ts`, API, UI déjà écrits mais jamais déployés ni la migration appliquée). Ajouté à `pricing_rules` : frais d'inscription (montant unique, par enfant ou par famille), `months_per_year` configurable (au lieu de 10 fixe partout), `sessions_per_month` réellement configurable (était déjà utilisé en dur, jamais exposé), `annual_discount_pct` (remise si paiement de l'année en une fois). Nouveau mode UI « Mensuel fixe par enfant » (raccourci pour un dégressif à 5 paliers égaux) en plus des 3 modes existants. `/settings/tarification` : bloc « Options » commun à tous les modes, description de règle et totaux annuels reflètent les vraies mensualités configurées. `NewRegistrationForm` : frais d'inscription et total annuel avec remise affichés en direct pendant la saisie. `generate-monthly` : le mode « par séance » facture désormais réellement (c'était un TODO « future » depuis le début). Migration appliquée en prod via le SQL Editor Supabase (blocage détecté par le classifieur auto-mode car modification de schéma partagé — confirmation explicite demandée et obtenue avant exécution). Vérifié en conditions réelles avec compte jetable : règle créée avec frais d'inscription 50€/famille + remise 5%, panneau de tarification de `/eleves/new` affiche correctement `40€ · +50€ inscription · 400€/an · 380€ si payé en une fois (-5%)`. Chaque organisation garde son tarif isolé par site (`organization_id`) — ces options s'ajoutent à l'intérieur de ce tarif, pas une nouvelle dimension de scoping. |
| 30 | 2026-07-16 | **Refonte premium de `/settings/tarification`** : l'utilisateur trouvait la page insuffisamment « pro, premium », pas au niveau visuel du reste de l'app. Repris le vocabulaire exact de `DashboardContent.tsx` (voir mémoire `exigence-ui-ultra-premium`) : hero 2 colonnes `rounded-2xl border-border bg-card` avec kicker + `HeroMetric` x4 (Sites, Tarifs actifs, Tarifs spéciaux, Total mensuel), panneau latéral « Répartition du chiffre d'affaires » (barres colorées aux vraies couleurs des sites, calcul %CA déjà existant côté serveur) + bloc « Total annuel estimé », `FadeIn` staggéré partout. Le sélecteur « Mode de facturation » (`<select>` brut) devient une grille de 4 cartes cliquables icône+description (même pattern que `QuickActionCard`), aucun changement de logique — juste l'affordance. Cartes de site : chip couleur du site en teinte (comme `SiteLoadRow`), mini barre %CA, hover `-translate-y-0.5`. Onglets restylés au pattern exact de `FinancesContent` (`bg-muted/40 p-1`). Aucune logique métier touchée (state/handlers copiés à l'identique). Vérifié fonctionnellement en conditions réelles (compte jetable : hero stats, sélecteur de mode, sauvegarde, affichage — tous corrects) ; couleurs vérifiées programmatiquement (`getComputedStyle` : fond `rgb(3,7,17)`, primaire `rgb(101,103,241)`, `border-radius: 16px` — identiques au Dashboard) car l'outil de capture d'écran était indisponible cette session (problème d'outil, sans rapport avec le code). |
| 31 | 2026-07-16 | **Contrôle CRUD complet des tarifs** : l'utilisateur « je dois pouvoir personnaliser moi-même toutes les tarifications, modifier, supprimer et en créer de tous genre ». La page ne gérait jusqu'ici qu'UN SEUL tarif par site (le plus récent actif), sans suppression possible — alors que le schéma (`name`, `effective_from`/`until`, `is_active`) était déjà conçu pour plusieurs tarifs par site. Ajouté `DELETE /api/pricing-rules/[id]` (manquait entièrement). `page.tsx` récupère désormais tous les tarifs par site (actifs et inactifs). L'UI affiche la liste complète par site avec statut Actif/Inactif, badge « En vigueur » sur celui réellement appliqué, dates de validité, Modifier/Supprimer par ligne, et « Nouveau tarif » pour en créer autant que voulu. Le formulaire gagne un nom éditable, une case Actif, et des dates de validité (à partir du / jusqu'au) — champs qui existaient côté API mais n'étaient jamais exposés. **Bug corrigé au passage** : `currentRuleOf()` retombait sur le premier tarif du site si aucun n'était actif, affichant à tort « En vigueur » sur un tarif inactif — retourne désormais `null`, un site sans tarif réellement actif affiche honnêtement « Non configuré ». Vérifié en conditions réelles (compte jetable) : 2 tarifs créés sur le même site (un actif, un inactif), modification/réactivation testée, suppression testée et confirmée (retour à « Non configuré » une fois le seul tarif actif supprimé). |
...
| 018 | `018_organizations.sql` | 🚧 **Multi-tenant** : table `organizations`, `organization_id` sur 26 tables + backfill « Teacher Khati », helpers `current_org_id()`/`is_org_writer()`, triggers org-fill (compat ancien code), uniques re-scopés par org, `handle_new_user()` self-service + seed, toutes les policies RLS recréées, Storage par org | ✅ **APPLIQUÉE (2026-07-08)** + 2 correctifs post-application (ERRORS/002, ERRORS/003) — code correspondant encore sur la branche |
